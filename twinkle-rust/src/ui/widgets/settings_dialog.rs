...
impl SettingsDialog {
    ...
    pub async fn new(
        ...
        let (general_tab, autostart_switch, theme_combo) =
            Self::build_general_tab(&original_config);
        let (ui_tab, auto_hide_spin, show_monitor_switch, enable_presets_switch) =
            Self::build_ui_tab(&original_config);
        let (behavior_tab, debounce_spin, remember_brightness_switch, restore_brightness_switch) =
            Self::build_behavior_tab(&original_config);
        let (advanced_tab, timeout_spin, retries_spin, debug_logging_switch) =
            Self::build_advanced_tab(&original_config);
    ...
    fn build_general_tab(config: &AppConfig) -> (Box, Switch, ComboBoxText) {
        ...
        let autostart_switch = Switch::builder().active(config.general.autostart).build();
        container.append(&Self::make_row(
            &autostart_switch,
    ...
    fn build_ui_tab(config: &AppConfig) -> (Box, SpinButton, Switch, Switch) {
        ...
        let auto_hide_label = Label::builder()
            .label("Auto-hide delay (ms, 0 = disabled):")
            .halign(gtk4::Align::Start)
            .build();
        let auto_hide_adj = Adjustment::new(
            config.ui.auto_hide_delay_ms as f64,
            0.0,
            30000.0,
            100.0,
            500.0,
        ...
        let show_monitor_switch = Switch::builder()
            .active(config.ui.show_monitor_selector)
            .build();
        container.append(&Self::make_row(
            &show_monitor_switch,
    ...
    fn build_behavior_tab(config: &AppConfig) -> (Box, SpinButton, Switch, Switch) {
        ...
        let debounce_adj = Adjustment::new(
            config.behavior.debounce_delay_ms as f64,
            0.0,
            1000.0,
            10.0,
            50.0,
        ...
        let remember_brightness_switch = Switch::builder()
            .active(config.behavior.remember_brightness)
            .build();
        container.append(&Self::make_row(
            &remember_brightness_switch,
            "Remember brightness per monitor",
        ));

        let restore_brightness_switch = Switch::builder()
            .active(config.behavior.restore_brightness)
            .build();
        container.append(&Self::make_row(
            &restore_brightness_switch,
    ...
    fn build_advanced_tab(config: &AppConfig) -> (Box, SpinButton, SpinButton, Switch) {
        ...
        let timeout_adj = Adjustment::new(
            config.advanced.command_timeout_secs,
            1.0,
            30.0,
            0.5,
            1.0,
        ...
        let retries_label = Label::builder()
            ...
            .build();
        let retries_adj =
            Adjustment::new(config.advanced.max_retries as f64, 0.0, 10.0, 1.0, 1.0, 0.0);
        let retries_spin = SpinButton::builder().adjustment(&retries_adj).build();
        let retries_row = Box::builder()
            .orientation(Orientation::Horizontal)
            .spacing(8)
            .build();
        ...
        let debug_logging_switch = Switch::builder()
            .active(config.advanced.debug_logging)
            .build();
        container.append(&Self::make_row(
            &debug_logging_switch,
